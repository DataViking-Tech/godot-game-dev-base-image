...
QUEUE_DIR = _queue_dir_for(_resolve_base_dir(None))
OUTPUT_DIR = _output_dir_for(_resolve_base_dir(None))
...
class GodotRenderBridge:
    ...
    def render_biome_showcase(
        ...
        Args:
            biome: One of crystal_vein, flats, frozen_lattice, basalt_reach
            camera: Camera preset (front, side, overhead, low, wide, closeup)
            distance: Camera distance in units
            density: Asset density (sparse, medium, dense, extreme)
            ...
            render_mode: normal, clay, or matcap
    ...
    def render_single_asset(
        ...
        Args:
            ...
            biome: Biome for lighting/ground colors
            camera: Camera preset
            distance: Camera distance in units
            terrain_mode: flat, procedural, or slope
            render_mode: normal, clay, or matcap
            output_width: Output image width
            output_height: Output image height
    ...
    def is_watcher_running(self) -> bool:
        ...
        # Look for recent activity in the output directory
        # or a heartbeat file from the watcher
        heartbeat = PROJECT_ROOT / "temp" / "godot-watcher-heartbeat"
        if heartbeat.exists():
    ...
    def cleanup_job(self, job_id: str) -> None:
        ...
        for pattern in [f"{job_id}.json", f"{job_id}_result.json", f"{job_id}.png"]:
            for directory in [QUEUE_DIR, OUTPUT_DIR]:
    ...
    def list_completed_jobs(self) -> list[str]:
        ...
        return [f.stem.replace("_result", "") for f in OUTPUT_DIR.glob("*_result.json")]
...
if __name__ == "__main__":
    ...
    parser.add_argument("--biome", default="crystal_vein", help="Biome to render")
